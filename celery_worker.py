...
def process_image_task(image, pdf_id, image_index, task_id):
    while True:
        try:
            ...
            print("Page: ", image_index, "got account: ", account_id, sep='-')
            time.sleep(5)
        except:
            print("Page: ", image_index, "Error in getting account", sep='-')
            time.sleep(30)
            continue
        try:
            try:
                ...
                print("Page: ", image_index, "Account", account_id, "Error in image processing", sep='-')
                print(traceback.format_exc())
                release_account(account_id, account_collection)
                continue
            if "Error" in image_data:
                print("Page: ", image_index, "Error in image data", sep='-')
            ...
            print("Page: ", image_index, "Sending image data to chat", sep='-')
            data_return = chat(cookie, image_data, headers)
            if "Error" in data_return:
                print("Page: ", image_index, "Error in data return", sep='-')
                release_account(account_id, account_collection)
                # recover_use(account_id, account_collection)
                time.sleep(random.randint(10, 30))
                continue
            if 'Wait' in data_return:
                print("Page: ", image_index, "Wait few seconds", sep='-')
                release_account(account_id, account_collection)
                recover_use(account_id, account_collection)
                continue
            if 'Quota' in data_return or 'Banned' in data_return:
                print("Page: ", image_index, "Account", account_id, "Quota exceeded", sep='-')
                release_account(account_id, account_collection)
                lock_account(account_id, account_collection)
                time.sleep(random.randint(10, 30))
                continue
            print("Page: ", image_index, "Done!", sep='-')
            release_account(account_id, account_collection)
            ...
            release_account(account_id, account_collection)
            print("Page: ", image_index, "Unknown error", sep='-')
            print(traceback.format_exc())
            continue
